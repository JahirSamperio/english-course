...
<body>
    <div class="manage-container">
        <div class="header bounce fade-in">
            <h1>📚 Gestionar Planes de Estudio</h1>
            <p>Edita y elimina planes existentes</p>
        </div>
                    ...
                    <?php if(isset($planes)): foreach($planes as $plan): ?>
                        <tr>
                            <td style="padding: 12px; border-bottom: 1px solid #ddd;">
                                <strong><?php echo $plan['titulo']; ?></strong>
                            </td>
                            <td style="padding: 12px; border-bottom: 1px solid #ddd;">
                            ...
                            <td style="padding: 12px; border-bottom: 1px solid #ddd;">
                                <?php echo $plan['duracion_semanas']; ?> semanas
                            </td>
                            <td style="padding: 12px; border-bottom: 1px solid #ddd;">
                                <?php echo substr($plan['descripcion'], 0, 50) . '...'; ?>
                            </td>
                            <td style="padding: 12px; border-bottom: 1px solid #ddd;">
                                <a href="/englishdemo/?controller=teacher&action=editPlan&id=<?php echo $plan['id']; ?>" class="btn-action btn-edit">
                                    ✏️ Editar
                                </a>
                                <a href="/englishdemo/?controller=teacher&action=deletePlan&id=<?php echo $plan['id']; ?>" 
            ...
            <?php if ($total_pages > 1): ?>
                <div class="pagination" style="text-align: center; margin-top: 20px;">
                    <?php if ($current_page > 1): ?>
                        <a href="?controller=teacher&action=managePlans&page=<?php echo $current_page - 1; ?>" class="btn-action btn-edit">← Anterior</a>
                    <?php endif; ?>
                    
                    <span style="margin: 0 15px; font-weight: bold;">Página <?php echo $current_page; ?> de <?php echo $total_pages; ?></span>
                    
                    <?php if ($current_page < $total_pages): ?>
                        <a href="?controller=teacher&action=managePlans&page=<?php echo $current_page + 1; ?>" class="btn-action btn-edit">Siguiente →</a>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        </div>

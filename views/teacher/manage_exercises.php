...
<body>
    <div class="manage-container">
        <div class="header bounce fade-in">
            ...
            <p>Edita y elimina ejercicios existentes</p>
        </div>
        ...
        <div class="exercises-table">
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    <?php if(isset($ejercicios)): foreach($ejercicios as $ejercicio): ?>
                        <tr>
                            <td style="padding: 12px; border-bottom: 1px solid #ddd;">
                                <strong><?php echo $ejercicio['titulo']; ?></strong>
                            </td>
                            <td style="padding: 12px; border-bottom: 1px solid #ddd;">
                                <?php echo ucfirst(str_replace('_', ' ', $ejercicio['tipo'])); ?>
                            </td>
                            <td style="padding: 12px; border-bottom: 1px solid #ddd;">
                                <span style="background: #4ECDC4; color: white; padding: 4px 8px; border-radius: 10px; font-size: 12px;">
                                    <?php echo $ejercicio['nivel']; ?>
                                </span>
                            </td>
                            <td style="padding: 12px; border-bottom: 1px solid #ddd;">
                                🏆 <?php echo $ejercicio['puntos']; ?>
                            </td>
                            <td style="padding: 12px; border-bottom: 1px solid #ddd;">
                                <a href="/englishdemo/?controller=teacher&action=editExercise&id=<?php echo $ejercicio['id']; ?>" class="btn-action btn-edit">
                                    ✏️ Editar
                                </a>
                                <a href="/englishdemo/?controller=teacher&action=deleteExercise&id=<?php echo $ejercicio['id']; ?>" 
            ...
            <?php if ($total_pages > 1): ?>
                <div class="pagination" style="text-align: center; margin-top: 20px;">
                    <?php if ($current_page > 1): ?>
                        <a href="?controller=teacher&action=manageExercises&page=<?php echo $current_page - 1; ?>" class="btn-action btn-edit">← Anterior</a>
                    <?php endif; ?>
                    
                    <span style="margin: 0 15px; font-weight: bold;">Página <?php echo $current_page; ?> de <?php echo $total_pages; ?></span>
                    
                    <?php if ($current_page < $total_pages): ?>
                        <a href="?controller=teacher&action=manageExercises&page=<?php echo $current_page + 1; ?>" class="btn-action btn-edit">Siguiente →</a>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        </div>
